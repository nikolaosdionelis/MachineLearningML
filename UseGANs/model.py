                    ...
                    for i in range(0, len(X_training), bsz):
                      ...
                      myNik_nlli = torch.from_numpy(np.asarray((
                        self.sess.run([self.log_likelihood],
                                      feed_dict={self.log_like_batch: out.cpu().detach().numpy()}))))
                      ...
                      #print('')
                      #train_gen_para, train_jac = self.flow_model(out.cpu().detach().numpy())
                      
                      # use: tf.convert_to_tensor(data_np, np.float32)
                      # now use: tf.convert_to_tensor(data_np, np.float32)
                      
                      #train_gen_para, train_jac = self.flow_model(out.cpu().detach().numpy())
                      train_gen_para, train_jac = self.flow_model(tf.convert_to_tensor(out.cpu().detach().numpy(), np.float32))

                      #train_nlli = nvp_op.log_likelihood(train_gen_para, train_jac,
                      #                                   batch_z) / config.batch_size2

                      train_nlli = nvp_op.log_likelihood(train_gen_para, train_jac,
                                                         config.prior) / config.batch_size
                      ...
                      #print(train_nlli.shape)
                      #print(train_nlli.requires_grad)
