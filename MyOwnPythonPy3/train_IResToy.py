...
if __name__ == '__main__':
    ...
    """
    checkpoint = torch.load(os.path.join(os.path.join(args.save, 'myOld'), 'unimode_checkpt5000.pth'))
    ...
    #checkpoint = torch.load(os.path.join(os.path.join(args.save, 'myOld'), 'checkpt-9000.pth'))
    #model.load_state_dict(checkpoint['state_dict'])
    #model.to(device)
    #model.eval()
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, 'nn2882myModelss0'), 'checkpt-%04d.pth' % args.niters2))
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, 'nn2882myModelss0'), 'checkpt-%04d.pth' % args.niters2), map_location=torch.device('cpu'))
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '00I0nn2882myModelss0'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    genGen.to(device)
    ...
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '0000IInn2882myModelss0'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr - 1) % 1000 == 0:
            with torch.no_grad():
                ...
                fig_filename = os.path.join(args.save, '00000IInn2882figs2', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '00000IInn2882myModelss0'), args.niters2)
        ...
        if lossGen < best_loss:
            ...
            utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                                  os.path.join(args.save, '00000IInn2882myModelss0Best'), args.niters2)
