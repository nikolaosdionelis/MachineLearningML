...
class DCGAN(object):
  ...
  def train(self, myMyMyData, config):
    seed = 0
    np.random.seed(seed)
    tf.set_random_seed(seed)
    """Train DCGAN"""
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, batch_idxs):
        ...
        if np.mod(counter, 703) == 1: #340
          ...
          indices_one = tesTest_labels != 2
          indices_zero = tesTest_labels == 2
    ...
    test_nllitest_nlli = self.test_model2(myMyMyData, config)
    return test_nllitest_nlli
  ...
  def test_model2(self, test_data, config):
    print("[*] Restoring best model counter: %d, val neg lli: %.8f"
          % (self.best_model_counter, self.best_val_nlli))
    self.saver.restore(self.sess, self.best_model_path)
    print("[*] Best model restore from: " + self.best_model_path)
