    ...
    for i in range(args.batch_size):
        for j in range(args.batch_size):
            if i != j:
                ...
                # third_term_loss += ((torch.norm(genFGen3[i,:] - genFGen3[j,:])) / (torch.norm(genFGen2[i,:] - genFGen2[j,:])))
                third_term_loss += ((torch.dist(genFGen3[i, :], genFGen3[j, :], 2)) / (
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        loss, logpz, delta_logp = compute_loss(args, model, beta=beta)
        #lossGen = loss_fn(first_term_loss, second_term_loss, third_term_loss)

        genFGen3 = torch.randn((args.batch_size, 2))
        genFGen2 = genGen.forward(genFGen3)
        ...
        # Add the three terms
        #all_terms_loss = first_term_loss + second_term_loss + third_term_loss
        ...
        #lossGen = loss_fn(first_term_loss, second_term_loss, third_term_loss)
        #lossGen = loss_fn2(first_term_loss, second_term_loss, third_term_loss)

        #lossGen = loss_fn2(first_term_loss, second_term_loss, third_term_loss)
        lossGen = loss_fn2(first_term_loss, genFGen2, args, model)

        #n = 256
        #(Variable(torch.rand((n, 2)), requires_grad=False)).detach().cpu()

        #Variable(torch.rand((n, 2)), requires_grad=False)
        #torch.rand((n, 2))

        #genGen = Generator(16)
        #genFGen2 = genGen.forward(torch.rand((256, 2)))

        #genFGen2 = genGen.forward(torch.rand((256, 2)))
        #genFGen2 = genGen.forward(Variable(torch.rand((256, 2)), requires_grad=False))

        #genFGen2 = genGen.forward(Variable(torch.rand((256, 2)), requires_grad=False))
        #genFGen2 = genGen.forward(torch.rand((256, 2)))

        #genFGen2 = genGen.forward(torch.rand((256, 2)))
        #genFGen2 = genGen.forward(torch.rand((args.batch_size, 2)))

        #genFGen2 = genGen.forward(torch.rand((args.batch_size, 2)))

        #genFGen3 = torch.rand((args.batch_size, 2))
        #genFGen2 = genGen.forward(genFGen3)

        #print('')
        #print(genGen.parameters())
        ...
        # rand or randn => Uniform or Gaussian
        # z space => rand or randn => Uniform or Gaussian
        ...
        # Generator(128) is a Tensor => batch_size x 2
        # Generator(128) is the same as args.data => batch_size x 2

        #first_term_loss = -compute_loss2(Generator(16), args, model, beta=beta)

        #genFGen2 = torch.FloatTensor(genFGen2.cpu())
        #genFGen2 = torch.from_numpy(genFGen2).type(torch.float32).to(device)
        ...
        plt.figure()
        plt.plot(genFGen3[:, 0].cpu().squeeze().numpy(), genFGen3[:, 1].cpu().squeeze().numpy(), 'o')
