    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr - 1) % 1000 == 0:
            with torch.no_grad():
                ...
                plt.text(5.3, -4.5 + 0.2, 'G(z) = CFS, B(z) = 2-8-8-2, {} epochs'.format(itr-1),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(5.3, -4.0 + 0.2, 'x ~ N(1), x std = 0.35, l1 = 0.3, l2 = 0.025, lr = {}'.format(args.lr),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(5.3, -3.5 + 0.2, 'LF value = {:.4f}, batch size = 256, sample size = 1024'.format(lossGen),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                # G(z): CFS, B(z): 2-8-8-2, epochs = 1000, batch size = 256, sample size = 1024,
                # l1 = 0.3, l2 = 0.025, lr = 0.01, x ~ N(1), x std = 0.35, LF value = A, LF gradient = A

                #plt.show()
                #plt.pause(0.1)

                fig_filename = os.path.join(args.save, '0nn2882figs2', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '0nn2882myModelss0'), args.niters2)
