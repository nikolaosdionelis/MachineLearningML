...
class DCDCDCGAN(object):
    ...
    def build_model(self, dcDcgan):
        ...
        #xData = tf.reshape(self.inputs, [-1, 28 * 28])
        xData = tf.reshape(self.inputs, [-1, 3 * 32 * 32])
        ...
        #genFGen2 = tf.reshape(self.G, [-1, 28 * 28])
        genFGen2 = tf.reshape(self.G, [-1, 3 * 32 * 32])
        ...
        geGeGenFGen2 = tf.reshape(genFGen2, [-1, 32, 32, 3])
        train_gen_para, train_jac = dcDcgan.flow_model(geGeGenFGen2)
        ...
        self.g0_loLoLoss = traTrain_nlli
        self.g1_loLoLoss = second_term_loLoss2
        self.g2_loLoLoss = third_term_loss12
        ...
        for epoch in xrange(config.epoch2):
            ...
            for idx in xrange(0, int(batch_idxs)):
                ...
                if config.dataset2 == 'mnist':
                    ...
                    e0errG = self.g0_loLoLoss.eval({
                        self.inputs: batch_images,
                        self.z: batch_z,
                        self.y: batch_labels
                    })

                    e1errG = self.g1_loLoLoss.eval({
                        self.inputs: batch_images,
                        self.z: batch_z,
                        self.y: batch_labels
                    })

                    e2errG = self.g2_loLoLoss.eval({
                        self.inputs: batch_images,
                        self.z: batch_z,
                        self.y: batch_labels
                    })
                ...
                print("[%8d Epoch:[%2d/%2d] [%4d/%4d] time: %4.4f, g_loss: %.8f, L0: %.8f, L1: %.8f, L2: %.8f" \
                      % (counter, epoch, config.epoch2, idx, batch_idxs,
                         time.time() - start_time, errG, e0errG, e1errG, e2errG))

                lossList_errG.append(errG)

                lossList_e0errG.append(e0errG)
                lossList_e1errG.append(e1errG)
                lossList_e2errG.append(e2errG)
                ...
                axs[0, 0].plot(range(1, 1 + epoch), lossList_errG[:epoch])
                axs[0, 0].set_title('Loss')
                # axs[0, 1].plot(range(1, 1 + epoch), lossList_e0errG[:epoch], 'tab:orange')
                axs[0, 1].plot(range(1, 1 + epoch), lossList_e0errG[:epoch])
                axs[0, 1].set_title('L0')
                # axs[1, 0].plot(range(1, 1 + epoch), lossList_e1errG[:epoch], 'tab:green')
                axs[1, 0].plot(range(1, 1 + epoch), lossList_e1errG[:epoch])
                axs[1, 0].set_title('L1')
                # axs[1, 1].plot(range(1, 1 + epoch), lossList_e2errG[:epoch], 'tab:red')
                axs[1, 1].plot(range(1, 1 + epoch), lossList_e2errG[:epoch])
                axs[1, 1].set_title('L2')
                #plt.savefig('toToSaSaveStore_imImaImImaImage')
                plt.savefig('toToToSaSaveSaveSaveStoStoStore_imImaImImaImage')
                plt.close('all')
