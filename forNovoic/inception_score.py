  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options\
    ...
    for op_idx, op in enumerate(ops):
        for o in op.outputs:
            ...
            #print(tf.TensorShape(new_shape))
            #asdfasdfasz
            ...
            # o.__dict__['_shape_val'] = tf.TensorShape(new_shape)
            # use: o.__dict__['_shape_val'] = tf.TensorShape(new_shape)

            #o._shape = tf.TensorShape(new_shape)
            #o.set_shape(new_shape)

            # Cannot feed value of shape (100, 32, 32, 3) for Tensor 'ExpandDims:0', which has shape '(1, ?, ?, 3)'
            # Cannot feed value of shape (100, 32, 32, 3) for Tensor 'ExpandDims:0', which has shape '(1, ?, ?, 3)'

            # Cannot feed value of shape (100, 32, 32, 3) for Tensor 'ExpandDims:0', which has shape '(1, ?, ?, 3)'
            # use: Cannot feed value of shape (100, 32, 32, 3) for Tensor 'ExpandDims:0', which has shape '(1, ?, ?, 3)'
    ...
    #print(tf.squeeze(pool3).shape)
    #print(tf.squeeze(pool3, [1, 2]).shape)
