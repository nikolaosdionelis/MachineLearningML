...
def presgan(dat, netG, netD, log_sigma, args, netG2):
    ...
    hmc_samples, acceptRate, stepsize, _ = hmc.get_samples(
        netG, X_training,
        torch.randn(batch_size, args.nz, 1, 1, device=device).clone(), sigma_x.detach(), args.burn_in,
        args.num_samples_posterior, args.leapfrog_steps, stepsize, args.flag_adapt,
        args.hmc_learning_rate, args.hmc_opt_accept)

    #hmc_samples, acceptRate, stepsize, _ = hmc.get_samples(
    #    netG2, varOutOut.detach(),
    #    gen_input.clone(), sigma_x.detach(), args.burn_in,
    #    args.num_samples_posterior, args.leapfrog_steps, stepsize, args.flag_adapt,
    #    args.hmc_learning_rate, args.hmc_opt_accept)

    # hmc_samples, acceptRate, stepsize, _ = hmc.get_samples(
    #    netG2, netG(torch.randn(batch_size, args.nz, 1, 1, requires_grad=True, device=device)), gen_input.clone(), sigma_x.detach(), args.burn_in,
    #    args.num_samples_posterior, args.leapfrog_steps, stepsize, args.flag_adapt,
    #    args.hmc_learning_rate, args.hmc_opt_accept)

    bsz, d = hmc_samples.size()
    mean_output = netG(hmc_samples.view(bsz, d, 1, 1).to(device))
    bsz = X_training.size(0)

    mean_output_summed = torch.zeros_like(X_training)
    ...
    #c = ((varOutOut - mean_output_summed) / sigma_x ** 2)
    c = ((X_training - mean_output_summed) / sigma_x ** 2)

    # c = ((netG(torch.randn(batch_size, args.nz, 1, 1, requires_grad=True,
    #                       device=device)) - mean_output_summed) / sigma_x ** 2).detach()

    # c = ((netG(torch.randn(batch_size, args.nz, 1, 1, requires_grad=True, device=device)) - mean_output_summed) / sigma_x ** 2).detach()
    # g_error_entropy = torch.mul(c, out + sigma_x * noise_eta).mean(0).sum()

    noise_eta = torch.randn_like(X_training)

    # use: X_training
    #g_error_entropy = torch.mul(c, out + sigma_x * noise_eta).mean(0).sum()

    #g_error_entropy = torch.mul(c, out + sigma_x * noise_eta).mean(0).sum()
    #g_error_entropy = torch.mul(c, X_training + sigma_x * noise_eta).mean(0).sum()

    #g_error_entropy = torch.mul(c, X_training + sigma_x * noise_eta).mean(0).sum()
    g_error_entropy = torch.mul(c, X_training + sigma_x * noise_eta).mean(0).mean()
    ...
    for _ in range(100):
        ndNdnikNikmyNikMy_entropy -= (
                (ndNdnikNikmyNikMy_entropy.clone() * torch.exp(ndNdnikNikmyNikMy_entropy.clone()) - myNikMy_entropy) / (
    ...
    #p_probP = torch.exp(gErrorEntropy2)
    #p_probP = gErrorEntropy2

    #print(torch.exp(p_probP))
    #asdfasdfda
    ...
    # (?)
    # p_probP = torch.zeros(1, device=device, requires_grad=True)
    # (?)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % args.save_imgs_every == 0:
            ...
            plt.xlim(0, epoch-1)
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossLoss_plot')

            plt.figure()
            plt.plot(loss_theLoss0.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossLoss0_plot')

            plt.figure()
            plt.plot(loss_theLoss1.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossLoss1_plot')

            plt.figure()
            plt.plot(loss_theLoss2.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossLoss2_plot')

            plt.figure()
            plt.plot(loss_theLoss3.cpu())
            plt.xlim(0, epoch - 1)
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossNewLoLossLoLossLoLossLoss2_plot')
            ...
            plt.savefig('dioDionelisWoWorkNdUoeMyMyNdNdWorkNeNewLossLossTotal_plot')
            ...
            plt.savefig('dioDionelisWoWoWorkNdUoeMyMyNdNdWorkNeNewLossLossTotal_plot')
