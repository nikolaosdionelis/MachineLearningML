...
def _helper(netG, x_tilde, eps, y, sigma):
...
def get_samples(netG, x_tilde, y_tilde, eps_init, sigma, burn_in, num_samples_posterior, 
    ...
    samples = torch.zeros(bsz*num_samples_posterior, eps_dim).to(device) # [128, 100]
    labels = torch.zeros(bsz*num_samples_posterior, y_tilde.size(1)) # [128, 10]
    ...
    for i in range(n_steps):
        ...
        current_p = p
        logjoint_vect, logjoint, grad_logjoint = _helper(netG, x_tilde, current_eps, current_y, sigma)
        ...
        for j in range(leapfrog_steps):
            ...
            if j < leapfrog_steps - 1:
                logjoint_vect, logjoint, grad_logjoint = _helper(netG, x_tilde, eps, current_y, sigma)
        ...
        if i < burn_in and flag_adapt == 1:
            ...
            labels[cnt*bsz : (cnt+1)*bsz, :] = current_y.squeeze() # ??? are we supposed to vary current_y as well?
            cnt += 1
        logjointHist[:, i] = -current_U.squeeze()
    acceptRate = acceptHist.mean(dim=1)
    return samples, labels, acceptRate, stepsize
