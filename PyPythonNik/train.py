...
def presgan(dat, netG, netD, log_sigma, args, netG2):
    ...
    for epoch in range(1, args.epochs+1):
        for i in range(0, len(X_training), bsz): 
            ...
            if i % args.log == 0:
                print(
                    'Epoch [%d/%d] .. Batch [%d/%d] .. Loss: %.4f .. L0: %.4f .. L1: %.4f .. L2: %.4f'
                    % (epoch, args.epochs, i, len(X_training), g_error.item(), firstOnly_lossGen.item(),
                       secondOnly_lossGen.item(), thirdOnly_lossGen.item()))
        ...
        if epoch % args.save_imgs_every == 0:
            ...
            plt.xlim(0, epoch-1)
            plt.savefig('neNewloLossLoss_plot')

            plt.figure()
            plt.plot(loss_theLoss0.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('neNewloLossLoss0_plot')

            plt.figure()
            plt.plot(loss_theLoss1.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('neNewloLossLoss1_plot')

            plt.figure()
            plt.plot(loss_theLoss2.cpu())
            plt.xlim(0, epoch-1)
            plt.savefig('neNewloLossLoss2_plot')
            ...
            plt.savefig('neNewloLossLossTotal_plot')
