...
def use_loss_fn2(genFGen2, args, model, genFGen3, toUse_storeAll, toUse_storeAll2):
    ...
    #print('')
    #first_term_loss.register_hook(lambda grad: print(grad.item()))

    #second_term_loss2.register_hook(lambda grad: print(grad.item()))
    #third_term_loss12.register_hook(lambda grad: print(grad.item()))
...
if __name__ == '__main__':
    ...
    #    os.path.join(os.path.join(args.save, '0nnnew_bs128_models2'), 'checkpt-%04d.pth' % args.niters2))

    checkpoint2 = torch.load(
        os.path.join(os.path.join(args.save, '00nnnew_bs128_models2'), 'checkpt-%04d.pth' % args.niters2))
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        #print(np.mean(genGen.lin1.weight.grad.tolist()) + np.mean(genGen.lin2.weight.grad.tolist()) + np.mean(
        #    genGen.lin3.weight.grad.tolist()))

        #print(np.mean(genGen.lin1.bias.grad.tolist()) + np.mean(genGen.lin2.bias.grad.tolist()) + np.mean(
        #    genGen.lin3.bias.grad.tolist()))

        print((torch.mean(genGen.lin1.weight.grad) + torch.mean(genGen.lin2.weight.grad) + torch.mean(
            genGen.lin3.weight.grad)).item())

        print((torch.mean(genGen.lin1.bias.grad) + torch.mean(genGen.lin2.bias.grad) + torch.mean(
            genGen.lin3.bias.grad)).item())

        print('')

        '''
        print(np.mean(genGen.lin1.weight.grad.tolist()))
        print(np.mean(genGen.lin2.weight.grad.tolist()))
        print(np.mean(genGen.lin3.weight.grad.tolist()))

        print('')

        print(np.mean(genGen.lin1.bias.grad.tolist()))
        print(np.mean(genGen.lin2.bias.grad.tolist()))
        print(np.mean(genGen.lin3.bias.grad.tolist()))
            ...
            with torch.no_grad():
                ...
                utils.save_checkpoint(
                    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                     'args': args},
                    os.path.join(args.save, '00nnnew_bs128_moMo2'), args.niters2)
