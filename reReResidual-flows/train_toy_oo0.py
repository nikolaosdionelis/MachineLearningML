...
def loss_fn2(genFGen2, args, model):
    ...
    second_term_loss = 3000.0 * torch.mean(second_term_loss2) / (args.batch_size**2)
    ...
    second_term_loss2 = 0.5 * torch.mean(second_term_loss32)
    ...
    third_term_loss12 = 0.008 * torch.mean(third_term_loss32)
...
if __name__ == '__main__':
    ...
    #optimizerGen = torch.optim.Adam(filter(lambda p: p.requires_grad, genGen.parameters()), lr=args.lr, weight_decay=args.weight_decay)
    #optimizerGen = torch.optim.SGD(genGen.parameters(), lr=args.lr, weight_decay=args.weight_decay)
