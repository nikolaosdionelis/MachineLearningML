...
def use_loss_fn2(genFGen2, args, model, genFGen3, toUse_storeAll, toUse_storeAll2):
    ...
    for i in range(args.batch_size):
        ...
        third_term_loss22 = (torch.sqrt((genFGen3[i, :] - genFGen3).pow(2).sum(1)).requires_grad_()) / (
                1.0e-17 + torch.sqrt((genFGen2[i, :] - genFGen2).pow(2).sum(1)).requires_grad_())
        third_term_loss32[i] = torch.mean(third_term_loss22)
    # third_term_loss12 = torch.mean(third_term_loss32)
...
if __name__ == '__main__':
    ...
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, 'myModelss0'), 'checkpt-%04d.pth' % args.niters2))
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr-1) % 500 == 0:
            with torch.no_grad():
                ...
                plt.text(4.3, -4.5 + 0.2, 'B(z) = MA, G(z) = CFS, x ~ N(1), x std = 0.35',
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(4.3, -4.0 + 0.2, '{} epochs, l1 = 1, l2 = 0, lr = {}'.format(itr-1, args.lr),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)
                ...
                utils.save_checkpoint({'state_dict': genGen.module.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                     'args': args}, os.path.join(args.save, 'myModelss0'), args.niters2)
