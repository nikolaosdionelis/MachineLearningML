...
if __name__ == '__main__':
    ...
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '00nl1models2'), 'checkpt-%04d.pth' % args.niters2))
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '00nl1models2'), 'checkpt-%04d.pth' % args.niters2), map_location=torch.device('cpu'))

    genGen.load_state_dict(checkpoint2['state_dict'])
    genGen.to(device)

    optimizerGen.load_state_dict(checkpoint2['optimizer_state_dict'])
    #optimizerGen.to(device)
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr - 1) % 1000 == 0:
            with torch.no_grad():
                ...
                plt.text(7.3, -4.5 + 0.2, 'G(z) = CFS, B(z) = 2-8-2, {} epochs'.format(itr-1),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(7.3, -4.0 + 0.2, 'x ~ N(1), x std = 0.35, l1 = 3, l2 = 0.025, lr = {}'.format(args.lr),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(7.3, -3.5 + 0.2, 'LF value = {:.4f}, batch size = 256, sample size = 1024'.format(lossGen),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)
                ...
                fig_filename = os.path.join(args.save, '00nl1figs2', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '00nl1models2'), args.niters2)
