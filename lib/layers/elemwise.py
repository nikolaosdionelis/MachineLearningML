...
class LogitTransform(nn.Module):
    ...
    def forward(self, x, logpx=None):
        ...
        #y = torch.log(s) - torch.log(1 - s)
        #y = torch.log(1e-17 + nn.ReLU(s)) - torch.log(1e-17 + nn.ReLU(1 - s))

        #y = torch.log(1e-17 + nn.ReLU(s)) - torch.log(1e-17 + nn.ReLU(1 - s))

        #y = torch.log(1e-17 + nn.ReLU(s)) - torch.log(1e-17 + nn.ReLU(1 - s))
        y = torch.log(1e-17 + torch.relu(s)) - torch.log(1e-17 + torch.relu(1 - s))
    ...
    def _logdetgrad(self, x):
        ...
        #logdetgrad = -torch.log(s - s * s) + math.log(1 - 2 * self.alpha)
        #logdetgrad = -torch.log(1e-17 + torch.relu(s - s * s)) + math.log(1 - 2 * self.alpha)

        #logdetgrad = -torch.log(1e-17 + torch.relu(s - s * s)) + math.log(1 - 2 * self.alpha)
        #logdetgrad = -torch.log(1e-17 + torch.relu(s - s * s)) + torch.log(1e-17 + torch.relu(1 - 2 * self.alpha))
