...
if __name__ == '__main__':
    ...
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '0nbs256myModelss0'), 'checkpt-%04d.pth' % args.niters2))
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '0nbs256myModelss0'), 'checkpt-%04d.pth' % args.niters2), map_location=torch.device('cpu'))
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr-1) % 1000 == 0:
            with torch.no_grad():
                ...
                plt.text(5.3, -3.5 + 0.2, 'LF value = {:.4f}, batch size = {}, sample size = 1024'.format(lossGen, args.batch_size),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)
                ...
                fig_filename = os.path.join(args.save, '00nbs256figs2', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '00nbs256myModelss0'), args.niters2)
