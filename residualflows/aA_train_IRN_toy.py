...
from lib.visualize_flow import visualize_transform3
...
class Generator(nn.Module):
   def __init__(self, nhidden):
       ...
       self.lin5 = nn.Linear(nhidden, nhidden)
       self.lin6 = nn.Linear(nhidden, nhidden)

       self.lin7 = nn.Linear(nhidden, 2)
   ...
   def forward(self, z):
       ...
       h = F.leaky_relu(self.lin1(z))
       #h = F.elu(self.lin1(z))
       #h = F.tanh(self.lin1(z))

       #x2 = F.relu(self.lin2(h))
       #x2 = torch.relu(self.lin2(h))
       x2 = F.leaky_relu(self.lin2(h))
       #x2 = F.elu(self.lin2(h))
       #x2 = F.tanh(self.lin2(h))

       #x2 = F.leaky_relu(self.lin3(x2))

       #x = self.lin3(x2)
       #x = self.lin4(x2)

       x3 = F.leaky_relu(self.lin3(x2))
       x4 = F.leaky_relu(self.lin4(x3))

       x5 = F.leaky_relu(self.lin5(x4))
       x6 = F.leaky_relu(self.lin6(x5))
...
if __name__ == '__main__':
    ...
    #optimizerGen = torch.optim.SGD(genGen.parameters(), lr=args.lr)
    optimizerGen = torch.optim.Adam(genGen.parameters(), lr=args.lr)

    #optimizerGen = optim.Adam(genGen.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    #optimizerGen = torch.optim.SGD(genGen.parameters(), lr=0.1, momentum=0.9)
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizerGen)
    ...
    #for itr in range(1, 5000):
        ...
        if itr % args.viz_freq == 0:
            with torch.no_grad():
                ...
                p_samples = toy_data.inf_train_gen(args.data, batch_size=100)
                ...
                visualize_transform(p_samples, torch.randn, standard_normal_logprob, transform=sample_fn,
                                    inverse_transform=density_fn, samples=True, npts=100, device=device)
                ...
                utils.save_checkpoint({'state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()},
                    os.path.join(args.save, '0mmodels'), args.niters)
    ...
    checkpoint = torch.load(os.path.join(os.path.join(cwd, 'experiments/0mmodels'), 'checkpt-5000.pth'))
    ...
    #checkpoint = torch.load(os.path.join(os.path.join(args.save), 'checkpt.pth'))
    checkpoint = torch.load(os.path.join(os.path.join(args.save), 'checkpt.pth'), map_location = torch.device('cpu'))
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, 'IRN_bs64models'), 'checkpt-%04d.pth' % args.niters2))
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, 'IRN_bs64models'), 'checkpt-%04d.pth' % args.niters2), map_location=torch.device('cpu'))
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '00IIRN_bs64models'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '000IIRN_bs64model771'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    ...
    #    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
    #    os.path.join(args.save, '0000IIIRRN_bs64model771'), args.niters2)

    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '0000IIIRRN_bs64model771'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    ...
    """
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '000000IIIIIRRNbs64model771'), 'checkpt-%04d.pth' % args.niters2))
    genGen.load_state_dict(checkpoint2['state_dict'])
    ...
    #mu = torch.from_numpy(np.array([2.82507515, 1.92882611e-04], dtype="float32")).to(device)
    #S = torch.from_numpy(np.array([[pow(0.07166782, 2), 0.0], [0.0, pow(0.06917527, 2)]], dtype="float32")).to(device)
    S = torch.from_numpy(np.array([[pow(0.35840544, 2), 0.0], [0.0, pow(0.34766033, 2)]], dtype="float32")).to(device)
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr-1) % 500 == 0:
            with torch.no_grad():
                ...
                #g2genFGen2 = visualize_transform3(xData, torch.randn, standard_normal_logprob, transform=sample_fn,
                #                    inverse_transform=density_fn, samples=True, npts=args.batch_size, device=device)

                g2genFGen2 = visualize_transform3(xData, torch.randn, standard_normal_logprob, transform=sample_fn,
                                                  inverse_transform=density_fn, samples=True, npts=args.batch_sizeM, device=device)

                plt.figure()

                plt.plot(xData[:, 0].cpu().squeeze().numpy(), xData[:, 1].cpu().squeeze().numpy(), '+r')
                plt.plot(g2genFGen2[:, 0], g2genFGen2[:, 1], 'xg')
                ...
                #plt.legend(['Real points', 'Generated points'])
                plt.legend(['Real points', 'IResNet Points', 'Generated points'])

                # 5000 epochs, l1 = 0.3, l2 = 0.025, lr = 1e-5,
                # B(z) = MA, G(z) = CFS, x ~ N(1), std = 0.35

                plt.text(7.3, -4.5 + 0.2, 'G(z) = IResNet, B(z) = 2-16-16-16-16-16-16-2, {} epochs'.format(itr-1),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(7.3, -4.0 + 0.2, 'x ~ N(2), x std = 0.35, l1 = 0.3, l2 = 0.25, lr = {}'.format(args.lr),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)

                plt.text(7.3, -3.5 + 0.2, 'LF value = {:.4f}, batch size = {}, sample size = {}'.format(lossGen, args.batch_size, args.batch_sizeM),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)
                ...
                fig_filename = os.path.join(args.save, '000000000000IIIIIIIRRNbs64fig771', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '000000000000IIIIIIIRRNbs64model771'), args.niters2)
        ...
        if lossGen < best_loss:
            ...
            utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                                  os.path.join(args.save, '000000000000IIIIIIIRRNbs64model771Best'), args.niters2)
