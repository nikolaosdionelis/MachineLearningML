...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.flow_model = tf.make_template('model',
                                           lambda x: nvp.model_spec(x, reuse=False, model_type=self.model_type,
                                                                    train=False,
                                                                    alpha=self.alpha, init_type=self.init_type,
                                                                    hidden_layers=self.hidden_layers,
                                                                    no_of_layers=self.no_of_layers,
                                                                    batch_norm_adaptive=1), unique_name_='model')
    ...
    def evaluate_neg_loglikelihood(self, data, config, batch_x, z):
        ...
        for idx in xrange(0, log_like_batch_idxs):
            ...
            lli = self.sess.run([self.log_likelihood],
                                feed_dict={self.log_like_batch: batch_images, disc_input: batch_x, gen_input: z})
