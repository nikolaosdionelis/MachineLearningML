...
torch.save({
    ...
    'ema': ema,
    'test_bpd': test_bpd,
}, os.path.join(args.save, 'models', 'mosttMosttRecent.pth'))
...
torch.save({
    ...
    'ema': ema,
    'test_bpd': test_bpd,
}, os.path.join(args.save, 'models', 'mostRecent.pth'))
...
#from torch.utils.tensorboard import SummaryWriter
#writer = SummaryWriter('./runs') # default is "runs"
...
# train_img.py --data cifar10
# --actnorm True --save experiments/cifar10
...
#parser.add_argument('--batchsize', help='Minibatch size', type=int, default=64)
parser.add_argument('--batchsizeM', help='Minibatch size', type=int, default=64)
...
#parser.add_argument('--save', help='directory to save results', type=str, default='experiment1')
#parser.add_argument('--save', help='directory to save results', type=str, default='experiments/cifar10')

#parser.add_argument('--save', help='directory to save results', type=str, default='experiments/cifar10')
parser.add_argument('--save', help='directory to save results', type=str, default='experiments/cifar')
...
#parser.add_argument('--resume', type=str, default='./experiment1/models/mMMRRRee9123rrreee9123rrreee9123rrree9123rrree9123.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/most_recent.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/most_recent.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/mostRecent.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/mostRecent.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/mosttMosttRecent.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/mosttMosttRecent.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/moosttMoosttRecentt.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/moosttMoosttRecentt.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/0mmoosttMoosttRecentt.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/moosttMoosttRecentt.pth')
#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/010mmoosttMoosttRecentt.pth')

#parser.add_argument('--resume', type=str, default='./experiments/cifar/models/010mmoosttMoosttRecentt.pth')
#parser.add_argument('--resume', type=str, default='./cifar10_resflow_16-16-16.pth')
...
#            'state_dict': model.state_dict(),
#            'optimizer_state_dict': optimizer.state_dict(),
#            'args': args, 'ema': ema, 'test_bpd': test_bpd,
...
class DCGANGeneratorGenerator(nn.Module):
    def __init__(self, nrand):
        super(DCGANGeneratorGenerator, self).__init__()
...
def use_loss_fn2(genFGen2, args, model, genFGen3, xData):
    ...
    xData = xData.view(-1, 3*32*32)

    #genFGen2 = genFGen2.view(-1, 28*28)
    genFGen2 = genFGen2.view(-1, 3*32*32)
...
#genGen = DCGANGenerator3(nrand)
#genGen = DCGANGeneratorGenerator(nrand)
...
if args.data == 'cifar10':
    ...
    trainset = datasets.CIFAR10(args.dataroot, train=True, transform=transform_train)
    ...
    args.batchsizeM = len(trainset)

    #print(args.batchsizeM)
    #adfasfas

    train_loader = torch.utils.data.DataLoader(
        trainset,
        batch_size=args.batchsizeM,
        shuffle=True,
        num_workers=args.nworkers,
    )
    ...
    args.val_batchsize = len(datasets.CIFAR10(args.dataroot, train=False, transform=transform_test))
    ...
    args.val_batchsize51 = len(datasets.CIFAR100(args.dataroot, train=False, transform=transform_test))

    test_loader51 = torch.utils.data.DataLoader(
        datasets.CIFAR100(args.dataroot, train=False, transform=transform_test),
        batch_size=args.val_batchsize51,
        shuffle=False,
        num_workers=args.nworkers,
    )

    '''
    args.val_batchsize61 = len(datasets.STL10(args.dataroot, split='test', transform=transform_test))

    test_loader61 = torch.utils.data.DataLoader(
        datasets.STL10(args.dataroot, split='test', transform=transform_test),
        batch_size=args.val_batchsize61,
        shuffle=False,
        num_workers=args.nworkers,
    )
    '''
    ...
    # use test_loaderSVHN
    # we use test_loaderSVHN

    '''
    args.val_batchsize2 = len(vdsets.SVHN(
    ...
    test_loaderSVHN = torch.utils.data.DataLoader(
        vdsets.SVHN(
            args.dataroot, split='test', download=True, transform=transforms.Compose([
                transforms.Resize(args.imagesize),
                transforms.ToTensor(),
        ...
        batch_size=args.val_batchsize2,
        shuffle=False,
        num_workers=args.nworkers,
    )

    # we use test_loaderSVHN
    # we now use test_loaderSVHN

    args.val_batchsize3 = len(vdsets.SVHN(
        args.dataroot, split='test', download=True, transform=transforms.Compose([
            transforms.Resize(args.imagesize),
            transforms.ToTensor(),
            add_noise,
    ...
    test_loaderSVHN2 = torch.utils.data.DataLoader(
        vdsets.SVHN(
            args.dataroot, split='test', download=True, transform=transforms.Compose([
                transforms.Resize(args.imagesize),
                transforms.ToTensor(),
        ...
        batch_size=args.val_batchsize3,
        shuffle=False,
        num_workers=args.nworkers,
    )
    '''
    ...
    #args.batchsizeImagenet = len(datasets.Imagenet32(train=True, transform=transforms.Compose([
    #        add_noise,
    #    ])))

    #train_loaderImagenet = torch.utils.data.DataLoader(
    #    datasets.Imagenet32(train=True, transform=transforms.Compose([
    #        add_noise,
    #    ])), batch_size=args.batchsizeImagenet, shuffle=True, num_workers=args.nworkers
    #)

    '''
    args.val_batchsizeImagenet = len(datasets.Imagenet32(train=False, transform=transforms.Compose([
            add_noise,
        ])))

    #print(args.val_batchsizeImagenet)
    #adfsdfasfsfsa

    test_loaderImagenet = torch.utils.data.DataLoader(
        datasets.Imagenet32(train=False, transform=transforms.Compose([
            add_noise,
        ])), batch_size=args.val_batchsizeImagenet, shuffle=False, num_workers=args.nworkers
    )
    '''

    # use train_loaderImagenet and test_loaderImagenet
    # we use train_loaderImagenet and test_loaderImagenet
...
def main():
    ...
    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '00000000mmmmmmainNikNik99MaaiiinMaaiiinBeBe'), 'checkpt-%04d.pth' % 1000))

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '000000000mmmmmmmaaiinnNikNik99MaaiiinMaaiiinBeBe'), 'checkpt-%04d.pth' % 10000))

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '0000000000mmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe'),
    #                 'checkpt-%04d.pth' % 10000))

    #0000000000mmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe
    #0000000000000000mmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '0000000000000000mmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe'),
    #                 'checkpt-%04d.pth' % 10000))

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '000000000000000mmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBe2Be2'),
    #                 'checkpt-%04d.pth' % 10000))

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '0000000000000000mmmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBe2Be2'),
    #                 'checkpt-%04d.pth' % 10000))

    #0000000000000000mmmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBe2Be2
    #0000000000000000mmmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBe2Be2
    ...
    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '00000000000000000mmmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe'),
    #                 'checkpt-%04d.pth' % 10000))

    #00000000000000000mmmmmmmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe
    #00000000000011111111111mmmmmmmmaainNikNik99MaaiiinMaaiiinBe2Be2

    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '00000000000011111111111mmmmmmmmaainNikNik99MaaiiinMaaiiinBe2Be2'),
    #                 'checkpt-%04d.pth' % 10000))
    ...
    #checkpoint2 = torch.load(
    #    os.path.join(os.path.join(args.save, '000000000001111111111mmmmmmmmaainNikNik99MaaiiinMaaiiinBe2Be2'),
    #                 'checkpt-%04d.pth' % 10000))
    ...
    #checkpoint2 = torch.load(
    #   os.path.join(os.path.join(args.save, '000000000000011111111111mmmmmmmmaainNikNik99MaaiiinMaaiiinBeBe'),
    #                'checkpt-%04d.pth' % 10000))
    ...
    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loader2):
    #for i, (x, y) in enumerate(test_loader3):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loader):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loaderSVHN):
    #for i, (x, y) in enumerate(test_loaderSVHN2):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loaderImagenet):
    #for i, (x, y) in enumerate(train_loaderImagenet):

    # we use: test_loaderImagenet
    #for i, (x, y) in enumerate(test_loaderImagenet):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loader51):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loader61):

    #for i, (x, y) in enumerate(test_loader):
    for i, (x, y) in enumerate(test_loader):

    #for i, (x, y) in enumerate(test_loader):
    #for i, (x, y) in enumerate(test_loader4):
        # print(y)
        ...
        #x2 = torch.empty(0, device=device)
        #y2 = torch.empty(0, device=device)
        #for i21 in range(len(y)):
        #    if y[i21] == 0:
        #        y2 = torch.cat((y2,y[i21]), dim=0)
        #        x2 = torch.cat((x2, x[i21,:,:,:]), dim=0)
        ...
        logger.info(
            'Itr {:} | Tm {:.2f}({:.2f}) | L {:.6f}({:.6f}) | L0 {:.6f}({:.6f}) | L1 {:.6f}({:.6f})  | L2 {:.6f}({:.6f})'.format(
                i, time2_meter.val, time2_meter.avg,
                loss2_meter.val, loss2_meter.avg, filoss2_meter.val, filoss2_meter.avg, seloss2_meter.val,
                seloss2_meter.avg, thloss2_meter.val, thloss2_meter.avg))
        ...
        plt.savefig('00000gggggggoooGGGGoo.png', bbox_inches='tight')
    ...
    plt.savefig('00000ffffffoooFFFFoo.png', bbox_inches='tight')
    ...
    for itr in range(1, args.nepochs + 1):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            gr1loss2_meter.update(
                (torch.mean(genGen.lin1.weight.grad) + torch.mean(
                    genGen.dc1.weight.grad) + torch.mean(genGen.dc2.weight.grad) + torch.mean(
                    genGen.dc3a.weight.grad) + torch.mean(genGen.lin1.bias.grad) + torch.mean(
                    genGen.dc1.bias.grad) + torch.mean(genGen.dc2.bias.grad) + torch.mean(genGen.dc3a.bias.grad)).item())
            ...
            if lossGen < best_loss:
                ...
                #    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                #     'args': args},
                #    os.path.join(args.save, '00000000mmmmmmainNikNik99MaaiiinMaaiiinBeBe'), args.nepochs)

                utils.save_checkpoint(
                    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                     'args': args},
                    os.path.join(args.save, '000000000000mmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBeBe'), args.nepochs)
            ...
            if (itr - 1) % 1000 == 0:
                ...
                #    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                #     'args': args},
                #    os.path.join(args.save, '0000000mmmmmmainNikNik99MaaiiinMaaiiinBe2Be2'), args.nepochs)

                utils.save_checkpoint(
                    {'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(),
                     'args': args},
                    os.path.join(args.save, '00000000000mmmmmmmmmmaaaiinnNikNik99MaaiiinMaaiiinBe2Be2'), args.nepochs)
    ...
    #plt.savefig('000ffffoooFFoo.png', bbox_inches='tight')
    plt.savefig('0000fffffoooFFoo.png', bbox_inches='tight')
