...
# train_img.py --data mnist --imagesize 28
# --actnorm True --wd 0 --save experiments/mnist
...
parser.add_argument('--mem-eff', type=eval, choices=[True, False], default=True)
#parser.add_argument('--mem-eff', type=eval, choices=[True, False], default=False)
...
model2.to(device)
#ema = utils.ExponentialMovingAverage(model2)
...
def train(epoch, model):
    ...
    # model2 is the offline model
    # and model is the proposed model
    ...
    fake_imgs = model(fixed_z, inverse=True).view(-1, *input_size[1:])
    #fake_imgs = model2(fixed_z, inverse=True).view(-1, *input_size[1:])
