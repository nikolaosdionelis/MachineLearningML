...
if __name__ == '__main__':
    ...
    #genGen = Generator(16).to(device)
    #genGen = Generator(128).to(device)
    ...
    #genGen = Generator(8).to(device)

    #genGen = Generator(8).to(device)
    #genGen = layers.SequentialFlow(blocks).to(device)
    ...
    #checkpoint = torch.load(os.path.join(os.path.join(args.save, 'myModels'), 'checkpt-%04d.pth' % itr))
    #checkpoint = torch.load(os.path.join(os.path.join(args.save, 'myModels'), 'checkpt-%04d.pth' % itr), map_location=torch.device('cpu'))

    #genGen.load_state_dict(checkpoint['state_dict'])
