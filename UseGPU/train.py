...
def presgan(dat, netG, netD, log_sigma, args, netG2):
    ...
    #fake = netG(fixed_noise).detach()
    #vutils.save_image(fake, '%s/presgan_%s_fake_epoch_%03d.png' % (args.results_folder, args.dataset, 0),
    ...
    #vutils.save_image(fake, '%s/prePresgan_%s_fake_epoch_%03d.png' % (args.results_folder, args.dataset, 0),
    #                  normalize=True, nrow=20)

    #fake = netG(fixed_noise).detach()
    #vutils.save_image(fake, '%s/prePrePresgan_%s_fake_epoch_%03d.png' % (args.results_folder, args.dataset, 0),
    #                  normalize=True, nrow=20)

    #torch.save(netG.state_dict(),
    #           os.path.join(args.results_folder, 'netG_prePresgan_%s_epoch_%s.pth' % (args.dataset, 0)))

    #torch.save(netG2.state_dict(),
    #           os.path.join(args.results_folder, 'netG_prePrePresgan_%s_epoch_%s.pth' % (args.dataset, 0)))

    #torch.save(netD.state_dict(),
    #           os.path.join(args.results_folder, 'netD_prePresgan_%s_epoch_%s.pth' % (args.dataset, 0)))

    #torch.save(log_sigma, os.path.join(args.results_folder, 'log_siSigma_%s_%s.pth' % (args.dataset, 0)))
    ...
    for epoch in range(1, args.epochs+1):
        for i in range(0, len(X_training), bsz): 
            ...
            if real_cpu.shape[0] != bsz:
                real_cpu = X_training[i-32:i + stop].to(device)
            ...
            #noise_eta2 = torch.randn_like(real_cpu)
            ...
            noise = torch.randn(batch_size, args.nz, 1, 1, device=device)
            mu_fake = netG(noise)
            noise_eta2 = torch.randn_like(mu_fake)
            #fake = mu_fake + sigma_x * noise_eta
            fake = mu_fake + sigma_x * noise_eta2
            #label.fill_(fake_label)
            ...
            gen_input = torch.randn(batch_size, args.nz, 1, 1, device=device, requires_grad=True)
            out = netG(gen_input)
            ...
            if args.lambda_ == 0:
                ...
                hmc_samples, acceptRate, stepsize = hmc.get_samples(
                    netG2, g_fake_data.detach(), gen_input.clone(), sigma_x.detach(), args.burn_in,
            ...
            if i % args.log == 0:
                print(
                    'Epoch [%d/%d] .. Batch [%d/%d] .. L: %.4f .. L0: %.4f .. L1: %.4f .. L2: %.4f'
                    % (epoch, args.epochs, i, len(X_training), g_error, firstOnly_lossGen, secondOnly_lossGen, thirdOnly_lossGen))
