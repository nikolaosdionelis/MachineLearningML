...
if __name__ == '__main__':
    ...
    checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '000IRN_bs64models'), 'checkpt-%04d.pth' % args.niters2))
    #checkpoint2 = torch.load(os.path.join(os.path.join(args.save, '000IRN_bs64models'), 'checkpt-%04d.pth' % args.niters2), map_location=torch.device('cpu'))
    ...
    for itr in range(1, args.niters2 + 1):
        ...
        if (itr-1) % 1000 == 0:
            with torch.no_grad():
                ...
                plt.text(7.3, -4.0 + 0.2, 'x ~ N(1), x std = 0.35, l1 = 0.3, l2 = 0.025, lr = {}'.format(args.lr),
                         verticalalignment='bottom', horizontalalignment='right', fontsize=10.5)
                ...
                fig_filename = os.path.join(args.save, '000IRN_bs64figs', 'ffii{:04d}.jpg'.format(itr))
                print('')
                ...
                utils.save_checkpoint({'state_dict': genGen.state_dict(), 'optimizer_state_dict': optimizerGen.state_dict(), 'args': args},
                    os.path.join(args.save, '000IRN_bs64models'), args.niters2)
