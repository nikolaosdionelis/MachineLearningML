              ...
              if np.mod(counter, 703) == 1:  # 340
                  ...
                  for epoch in range(1, epEpochs + 1):
                    for i in range(0, len(X_training), bsz):
                      ...
                      myNik_nlli2 = (self.sess.run([self.log_likelihood],
                                                  feed_dict={self.log_like_batch: batch_images.cpu().detach().numpy()}))

                      #myNik_nlli = torch.from_numpy(myNik_nlli2)
                      #torch.as_tensor(np.array(pil_img).astype('float'))

                      #myNik_nlli = torch.from_numpy(myNik_nlli2)
                      #myNik_nlli = torch.as_tensor(np.array(myNik_nlli2).astype('float'))

                      # use: np.gradient(
                      #myNik_nlli = torch.as_tensor(np.gradient(np.array(myNik_nlli2).astype('float')))

                      #myNik_nlli = torch.as_tensor(np.gradient(np.array(myNik_nlli2).astype('float')))
                      #myNik_nlli = torch.as_tensor((np.array(myNik_nlli2).astype('float')))

                      print((np.array(myNik_nlli2).astype('float')))
                      ...
                      #myNik_nlli = torch.from_numpy(np.gradient(
                      #  self.sess.run([self.log_likelihood], feed_dict={self.log_like_batch: batch_images.numpy()})))
