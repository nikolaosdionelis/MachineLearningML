...
class GANDCGAN2(object):
  def __init__(self, sess, input_height=32, input_width=32,
    ...
    Args:
      ...
      batch_size: The size of batch which should be specified before training
  ...
  def build_model(self):
    ...
    self.flow_model = tf.make_template('model2',
    ...
    self.trainable_flow_model = tf.make_template('model2',
                                                 lambda x: nvp.model_spec(x, reuse=True, model_type=self.model_type,
                                                                          train=True,
                                                                          alpha=self.alpha, init_type=self.init_type,
                                                                          hidden_layers=self.hidden_layers,
                                                                          no_of_layers=self.no_of_layers,
    ...
    self.flow_inv_model = tf.make_template('model2',
                                           lambda x: nvp.inv_model_spec(x, reuse=True, model_type=self.model_type,
                                                                        train=True, alpha=self.alpha),
                                           unique_name_='model2')
    # ##### f^-1: Latent to image (not-trainable just for sampling)#######
    self.sampler_function = tf.make_template('model2',
                                             lambda x: nvp.inv_model_spec(x, reuse=True, model_type=self.model_type,
                                                                          alpha=self.alpha, train=False),
                                             unique_name_='model2')
    ...
    #self.D, self.D_logits = tf.zeros(inputs), tf.zeros(inputs)
    #self.D, self.D_logits = 0.0, 0.0

    self.D_, self.D_logits_ = self.discriminator(self.G, reuse=False)
    #self.D_, self.D_logits_ = tf.zeros(self.G), tf.zeros(self.G)
    #self.D_, self.D_logits_ = 0.0, 0.0
      ...
      for idx in xrange(0, batch_idxs):
        ...
        if np.mod(counter, 703) == 1:  # 340
          ...
          print("[LogLi (%d,%d)]: val neg logli: %.8f, ince: %.8f, train lli: %.8f" % (epoch, idx, val_nlli, \
                                                                                       curr_inception_score[0], np.mean(
